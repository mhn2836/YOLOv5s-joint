    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # iou = bbox_alpha_iou(pbox.T, tbox[i], x1y1x2y2=False, alpha=4, CIoU=True) # use alpha IoU
...
class ComputeJLoss:
    ...
    def __init__(self, model, joint = True, autobalance=False):
        self.sort_obj_iou = False
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
        ...
        # per loss
        self.per_loss = LossNetwork(vgg_model).cuda()
        self.per_loss.eval()
    ...
    def __call__(self, p, targets, gt = None, dehaze = None):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets

        lde = torch.zeros(1, device=device)
        lde += self.dehaze_loss(gt, dehaze) #loss dehaze
        ...
        if self.joint is False:
            return (lbox + lobj + lcls) * bs, torch.cat((lbox, lobj, lcls)).detach()
        else:
            loss_det = lbox + lobj + lcls
            loss_dehaze = lde
            lammda = 5

            total_loss = (lammda * loss_dehaze + loss_det) * bs

            #print(lbox)
            #print(lde)

            return total_loss, torch.cat((lbox, lobj, lcls, lde)).detach()
    ...
    def dehaze_loss(self, gt, dehaze):
        total_loss = 0
        '''
        # per loss
        vgg_model = vgg16(pretrained=True).features[:16]
        ...
        loss_network.eval()
        '''
        smooth = self.mae(dehaze, gt)
        #ssim = self.ssim_loss(dehaze, gt)
        # perceptual_loss = loss_network(dehaze, gt)
        perceptual_loss = self.per_loss(dehaze, gt)
        ...
        total_loss = 2 * smooth + 1 * perceptual_loss
