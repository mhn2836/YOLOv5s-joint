...
from unet_model_mini import UNet_mini,UNet_mini_sk, UNet_mini_ska
...
class Slice(nn.Module):
    ...
    def forward(self, bilateral_grid, guidemap): 
        ...
        # hg = hg.half().repeat(N, 1, 1).unsqueeze(3) / (H - 1)
        wg = wg.float().repeat(N, 1, 1).unsqueeze(3) / (W-1) # norm to [0,1] NxHxWx1
        # wg = wg.half().repeat(N, 1, 1).unsqueeze(3) / (W - 1)
        hg, wg = hg*2-1, wg*2-1
...
class B_transformer(nn.Module):
    def __init__(self):
        ...
        self.u_net_mini = UNet_mini_sk(n_channels=3)
        # self.u_net_mini = UNet_mini_ska(n_channels=3)
    ...
    def forward(self, x):
        ...
        coeff = self.downsample(self.u_net_mini(x_r)).reshape(-1, 12, 16, 16, 16)
