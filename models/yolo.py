...
from models.evc_blocks import *
from models.Models.Attention.GAMAttention import *
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(cfg, dict):
            ...
            if cfg.find("joint") != -1:
                self.ori_file = Path(cfg[:-11] + '.yaml').name
                with open(cfg[:-11] + '.yaml', encoding='ascii', errors='ignore') as f:
                    self.ori_yaml = yaml.safe_load(f)  # model dict
                    self.ori_model, self.ori_save = parse_model(deepcopy(self.ori_yaml), ch=[ch])
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        if isinstance(self.model[0], Pre_encoder):
            # print('joint net')
            '''
            pre = Pre_encoder()
            pre.load_state_dict(torch.load('../4kdehaze.pth'))
            # self.pre_encoder.load_state_dict(torch.load('../4kdehaze.pth'))
            x = pre(x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                SCConv, DeformConv2D, DCNv2, C3DCN,
            ...
            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, C3HB, C3RFEM, C3STR, C3DCN]:
                args.insert(2, n)  # number of repeats
                n = 1
        # add module research
        elif m in [CARAFE, SPPCSPC, SPPFCSPC, RepConv, BoT3, CA, CBAM, NAMAttention, GAMAttention, Involution, Stem, ResCSPC, ResCSPB, \
                   ResXCSPB, ResXCSPC, BottleneckCSPB, BottleneckCSPC,EVCBlock,SwinV2_CSPB,SCBottleneck, C3SC,
            ...
            if m in [C3RFEM, SPPCSPC, BoT3, ResCSPC, ResCSPB, ResXCSPB, ResXCSPC, BottleneckCSPB, BottleneckCSPC, \
                HorBlock, CNeB, C3GC, C3C2, SwinV2_CSPB, SCBottleneck, C3SC]:
                args.insert(2, n)  # number of repeats
                n = 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='../configs/yolov5s_joint.yaml', help='model.yaml')
